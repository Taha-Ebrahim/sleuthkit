...
FILE* fs_old_output = tsk_set_printf_fd(tsk_make_tempfile());
FILE* fs_old_err = tsk_set_stderr_fd(tsk_make_tempfile());
...
struct CleanupFsTempFiles {
    ~CleanupFsTempFiles() {
        if (fs_old_output) fclose(fs_old_output);
        if (fs_old_err) fclose(fs_old_err);
    }
};

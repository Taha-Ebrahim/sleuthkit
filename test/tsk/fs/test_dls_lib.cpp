...
FILE* old_dls_out = tsk_set_printf_fd(tsk_make_tempfile());
FILE* old_dls_err = tsk_set_stderr_fd(tsk_make_tempfile());
...
struct CleanupDlsTempFiles {
    ~CleanupDlsTempFiles() {
        if (old_dls_out) fclose(old_dls_out);
        if (old_dls_err) fclose(old_dls_err);
    }
};
